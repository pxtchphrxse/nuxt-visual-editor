...
const openHyperlinkInNewTab = ref<boolean | null>(false)
...
watch(hyperlinkEnable, (hyperlinkEnableNewValue) => {
  ...
  state.hyberlinkEnable = hyperlinkEnable.value
})
...
const handleHyperlink = function () {
  designer.handleHyperlink(
    ...
    openHyperlinkInNewTab.value ?? undefined,
  )
}
    ...
    <template #content>
      ...
      <div v-if="getHyperlinkAbility === true">
        ...
        >
          <div class="flex items-center">
            <div class="flex-shrink-0">
              <CheckIcon class="w-4 h-4 text-myPrimaryLinkColor" />
            </div>
            <div class="ml-2">
              <p class="text-sm font-medium text-green-800">
                Hyperlink added
